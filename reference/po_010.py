...
from core.po_common import (
    PO_INPUT_PREFIX,
    PR_LINES_INPUT_PREFIX,
    find_period_input_file,
    normalize_company,
...
def _read(
    ...
    if raw.empty:
        raise ValueError(
            f"{CONTROL_ID}: {source_name} no contiene datos."
        )
    ...
    if missing:
        raise ValueError(
            f"{CONTROL_ID}: faltan headers en {source_name}: {missing}. "
    ...
    if source_name == "PO Lines":
        vendor_column = next(
            (
                column
                for column in VENDOR_NAME_HEADERS
...
def _filter_scope(
    ...
    if invalid_date.any():
        raise ValueError(
            f"{CONTROL_ID}: {source} contiene "
            f"{int(invalid_date.sum())} "
    ...
    if date_from > date_to:
        raise ValueError(
            f"{CONTROL_ID}: CONFIG FROM "
            f"{date_from.date()} es posterior "
            f"a TO {date_to.date()}."
        )
    ...
    result = (
        result.loc[date_mask]
        .copy()
    )

    result[date_column] = (
        parsed_date.loc[date_mask]
    )
...
def _deduplicate(
    ...
    if conflicts.any():
        ...
        raise ValueError(
            f"{CONTROL_ID}: {source} contiene "
            f"{len(conflicting_keys)} claves "
            "duplicadas con datos diferentes. "
            "Ejemplos:\n"
...
def _load_po(context):
    ...
    dataframe, metrics = _read(
        context=context,
        input_prefix=PO_INPUT_PREFIX,
        source_name="PO Lines",
        columns=PO_COLUMNS,
    )
    ...
    missing_pr_number = pr_number.isin(
        {
            "",
            "0",
        }
    ...
    if incomplete_link.any():
        ...
        raise ValueError(
            ...
            f"{int(incomplete_link.sum())} "
            "referencias parciales a PR. "
            "Ejemplos:\n"
            f"{sample.to_string(index=False)}"
        )

    dataframe = (
        dataframe.loc[~without_pr]
        .copy()
    )
...
def _load_pr(context):
    ...
    if partial.any():
        ...
        raise ValueError(
            f"{CONTROL_ID}: PR Lines contiene "
            f"{int(partial.sum())} "
            "claves parciales. "
            "Ejemplos:\n"
            f"{sample.to_string(index=False)}"
...
def _exceptions(
    ...
    metrics = {
        ...
        "PO rows without matching PR": (
            len(po)
            - len(joined)
        ),
        "Rows with item difference": int(
            item_difference.sum()
        ),
        "Rows with quantity difference": int(
...
def run_po_010(context):
    ...
    po, po_metrics = _load_po(
        context
    )

    timing["PO load/prepare"] = (
        perf_counter()
        - started
    )
    ...
    pr, pr_metrics = _load_pr(
        context
    )

    timing["PR load/prepare"] = (
        perf_counter()
        - started
    )
    ...
    result, analytic_metrics = _exceptions(
        po,
        pr,
    )
