...
def find_gl_fx_rates_input_file(context):
    return find_period_file_using_keywords(
        context=context,
        keyword_prefixes=GL_FX_INPUT_KEYWORDS,
    )
...
def load_gl_fx_rates_data(context):
    input_file = find_gl_fx_rates_input_file(context)

    if input_file is None:
        print("GL FxRates input file: not found. USD columns will be blank.")
        return pd.DataFrame()

    print(f"GL FxRates input file: {input_file}")
...
def resolve_fx_columns(fx_dataframe):
    return require_columns(
        dataframe=fx_dataframe,
        required_columns={
            "rate_type": [
                "CgCâ",
                "CgCa",
        ...
        source_name="GL FxRates",
    )


def get_optional_fx_factor_columns(fx_dataframe):
    return {
        "factor_from": get_optional_column(
            fx_dataframe,
            [
                "Fator (origem)",
                "Fator origem",
...
def normalize_fx_rates(fx_dataframe):
    ...
    required_columns = resolve_fx_columns(fx_dataframe)
    optional_columns = get_optional_fx_factor_columns(fx_dataframe)

    result = pd.DataFrame()

    result["Rate Type"] = (
        fx_dataframe[required_columns["rate_type"]]
        .apply(normalize_text)
        .str.upper()
    )
    result["From Currency"] = (
        fx_dataframe[required_columns["from_currency"]]
        .apply(normalize_text)
        .str.upper()
    )
    result["To Currency"] = (
        fx_dataframe[required_columns["to_currency"]]
        .apply(normalize_text)
        .str.upper()
    )
    result["Valid From"] = fx_dataframe[required_columns["valid_from"]].apply(to_datetime_value)
    result["Rate"] = fx_dataframe[required_columns["rate"]].apply(parse_number)

    factor_from_column = optional_columns["factor_from"]
    factor_to_column = optional_columns["factor_to"]

    if factor_from_column is None:
        result["Factor From"] = 1
    else:
        result["Factor From"] = fx_dataframe[factor_from_column].apply(parse_number).fillna(1)

    if factor_to_column is None:
        result["Factor To"] = 1
    else:
        result["Factor To"] = fx_dataframe[factor_to_column].apply(parse_number).fillna(1)

    result = result.dropna(subset=["Valid From"]).copy()
    result["Valid From"] = result["Valid From"].dt.normalize()

    return result


def calculate_adjusted_fx_rate(rate, factor_from, factor_to):
    if pd.isna(rate):
        return pd.NA

    rate = float(rate)

    if pd.isna(factor_from) or factor_from == 0:
        factor_from = 1

    if pd.isna(factor_to) or factor_to == 0:
        factor_to = 1

    return rate * float(factor_to) / float(factor_from)


def build_fx_date_candidates(requested_date, max_previous_days=10):
    ...
    return [
        requested_date - pd.Timedelta(days=days_back)
        for days_back in range(0, max_previous_days + 1)
    ]


def select_fx_rate_to_usd(normalized_fx_dataframe, currency, requested_date):
    ...
    if currency == "USD":
        return {
            "fx_to_usd": 1.0,
            "usd_rate": 1.0,
            "rate_date": requested_date.normalize(),
            "method": "FxRates: USD currency, rate 1.000000",
        }

    if normalized_fx_dataframe.empty:
        return None

    rate_type_priority = ["EN", "M"]

    for rate_type in rate_type_priority:
        for candidate_date in build_fx_date_candidates(requested_date):
            candidate_rates = normalized_fx_dataframe[
                (normalized_fx_dataframe["Rate Type"] == rate_type)
                & (normalized_fx_dataframe["Valid From"] == candidate_date)
            ]

            if candidate_rates.empty:
                continue

            direct_rates = candidate_rates[
                (candidate_rates["From Currency"] == currency)
                & (candidate_rates["To Currency"] == "USD")
            ]

            if not direct_rates.empty:
                row = direct_rates.iloc[0]

                adjusted_rate = calculate_adjusted_fx_rate(
                    rate=row["Rate"],
                    factor_from=row["Factor From"],
                    factor_to=row["Factor To"],
                )

                if pd.isna(adjusted_rate) or adjusted_rate == 0:
                    continue

                fx_to_usd = float(adjusted_rate)
                usd_rate = 1 / fx_to_usd

                return {
                    "fx_to_usd": fx_to_usd,
                    "usd_rate": usd_rate,
                    "rate_date": candidate_date,
                    "method": f"FxRates {rate_type}: {currency}->USD; USD = amount * FX",
                }

            inverse_rates = candidate_rates[
                (candidate_rates["From Currency"] == "USD")
                & (candidate_rates["To Currency"] == currency)
            ]

            if not inverse_rates.empty:
                row = inverse_rates.iloc[0]
                ...
                usd_rate = float(adjusted_rate)
                fx_to_usd = 1 / usd_rate

                return {
                    "fx_to_usd": fx_to_usd,
                    "usd_rate": usd_rate,
                    "rate_date": candidate_date,
                    "method": f"FxRates {rate_type}: USD->{currency}; USD = amount / USD Rate",
                }
