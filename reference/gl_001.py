...
def parse_date_series(series):
    """
    Vectorized date parser for SAP/Excel dates.
    ...
    This avoids slow row-by-row apply(to_datetime_value).
    """
    ...
    yyyymmdd_mask = (
        ~blank_mask
        & text_series.str.fullmatch(r"\d{8}", na=False)
    )

    yyyy_mm_dd_mask = (
        ~blank_mask
        & text_series.str.fullmatch(r"\d{4}-\d{2}-\d{2}.*", na=False)
    )
...
def prepare_source_dataframe(source_dataframe, context, source_name):
    ...
    if optional_columns["reversal_document"] is None:
        print(f"WARNING: {source_name} reversal document column was not detected.")

    if optional_columns["reversal_year"] is None:
        print(f"WARNING: {source_name} reversal year column was not detected.")

    if optional_columns["reversal_reason"] is None:
        print(f"WARNING: {source_name} reversal reason column was not detected.")
...
def add_header_totals(line_dataframe):
    ...
    if "Line Amount USD" in result.columns:
        line_amount_usd = pd.to_numeric(
            result["Line Amount USD"],
            errors="coerce",
        )

        header_total_usd_by_trans_id = (
            line_amount_usd
            .abs()
            .fillna(0)
            .groupby(result["TransId"])
        ...
        result["Header Total USD"] = result["TransId"].map(
            header_total_usd_by_trans_id
        )
        result["Header Total USD"] = result["Header Total USD"].where(
            result["TransId"].map(has_usd_by_trans_id),
            pd.NA,
        )
...
def complete_reversal_fields_by_trans_id(line_dataframe):
    """
    Complete reversal fields at TransId level before converting the result to header-level.

    This prevents losing reversal information when the first line kept by drop_duplicates
    has blank reversal fields but another line in the same TransId has values.
    """
    result = line_dataframe.copy()
    ...
    for column_name in reversal_columns:
        result[column_name] = (
            result
            .groupby("TransId")[column_name]
            .transform(first_non_blank_value)
        )

    reversal_document = normalize_code_series(result["SAP Reversal Document"])
    reversal_year = normalize_code_series(result["SAP Reversal Year"])

    result["Reverses TransId"] = build_reverses_trans_id_series(
        company_code=result["CoCo"],
        reversal_year=reversal_year,
        reversal_document=reversal_document,
    )

    result["Journal Entry Status"] = result["Journal Entry Status"].mask(
        reversal_document != "",
        "Reversed",
    )
...
def create_gl01_weekend_journals(
    bsis_dataframe,
    bsas_dataframe,
    master_dataframe,
    fx_dataframe,
    ...
    weekend_line_dataframe = add_usd_fields(
        line_dataframe=weekend_line_dataframe,
        fx_dataframe=fx_dataframe,
    )

    weekend_line_dataframe = add_header_totals(weekend_line_dataframe)

    weekend_line_dataframe = complete_reversal_fields_by_trans_id(
        weekend_line_dataframe
    )
...
def run_gl_001(context):
    ...
    print("Creating GL01 weekend journals with optimized FX and reversal logic...")
