...
PO01_SOURCE_MAPPING = {
    ...
    'PO Doc Date': 'PO Doc Date',
    'PO Doc Currency': 'PO Doc Currency',
    'Item Code': 'Item Code',
    'PO Material Description': 'PO Material Description',
    'PO Quantity': 'PO Quantity',
    'PO Unit Price': 'PO Unit Price',
    'PO Line Total': 'PO Line Total',
    'PO Creator ID': 'PO Creator ID',
    'PR Line': 'PR Line',
    'SPLIT_PO_KEY': 'SPLIT_PO_KEY',
...
def prepare_creator_population(dataframe):
    ...
    resolved_creator = (
        prepared
        .assign(_CREATOR=creator_for_group)
        .groupby(
            po_key,
            dropna=False,
        )['_CREATOR']
    ...
    po_creator_status = (
        prepared
        .assign(_HAS_CREATOR=line_has_creator)
        .groupby(
            po_key,
            dropna=False,
        )['_HAS_CREATOR']
    ...
    rows_with_creator = int(line_has_creator.sum())
    pos_with_creator = int(po_creator_status.sum())
...
def creator_coverage_is_sufficient(metrics):
    ...
    return (
        metrics['creator_line_coverage'] >= MINIMUM_CREATOR_COVERAGE
        and metrics['creator_po_coverage'] >= MINIMUM_CREATOR_COVERAGE
    )
...
def prepare_split_scope(dataframe):
    ...
    metrics = {
        ...
        'rows_excluded_by_logic': int((~scope_mask).sum()),
    }
...
def exclude_missing_creators(
    ...
    metrics['rows_eligible_for_logic'] = len(eligible)

    metrics['rows_excluded_by_logic'] += int(
        (~creator_mask).sum()
    )
...
def find_split_documents(
    ...
    if dataframe.empty:
        return pd.DataFrame(
            columns=GROUP_FIELDS + ['PO Number']
        )
    ...
    for group_values, group in grouped_documents:
        ...
        for index in range(1, len(group)):
            ...
            if difference <= window_days:
                flagged_positions.add(index - 1)
                flagged_positions.add(index)
        ...
        for index in sorted(flagged_positions):
            flagged_keys.append(
                group_tuple
                + (
                    group.at[
...
def build_po01_exceptions(
    ...
    exceptions['Split Window Days'] = window_days
...
def build_po01_output(exceptions):
    """Align PO01 detail to LHA columns without fabricating unavailable data."""
    output = pd.DataFrame(
        '',
        index=exceptions.index,
        columns=PO01_OUTPUT_COLUMNS,
    )

    for output_column, source_column in PO01_SOURCE_MAPPING.items():
        if source_column in exceptions.columns:
            output[output_column] = exceptions[source_column]
...
def build_summary_row(
    ...
    return {
        ...
        'Rows After Config Filters': (
            input_metrics['rows_after_config_filters']
        ),
        'Distinct POs': creator_metrics['distinct_pos'],
        'Rows With Creator': creator_metrics['rows_with_creator'],
        'Rows Without Creator': creator_metrics['rows_without_creator'],
        'Creator Line Coverage': creator_metrics[
            'creator_line_coverage'
        ],
        'POs With Creator': creator_metrics['pos_with_creator'],
        'POs Without Creator': creator_metrics['pos_without_creator'],
...
def print_po01_metrics(
    ...
    print(f"Header row: {input_metrics['header_row']}")
...
def run_po_001(context):
    ...
    coverage_sufficient = creator_coverage_is_sufficient(
        creator_metrics
    )
